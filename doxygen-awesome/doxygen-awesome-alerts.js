...
class DoxygenAwesomeAlerts {
  ...
  static get_alert(config) {
    const alert = document.createElement('dt');

    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('fill', `var(--${config['class']}-color-dark)`);
    ...
    title.setAttribute('style', 'line-height: 16px; vertical-align: top;')
  ...
  static init() {
    $(() => {
      let blockquotes = Array.from(document.getElementsByTagName('blockquote'));
      for (let blockquote of blockquotes) {
        ...
        first_paragraph.innerHTML = text.slice(tag.length).trimStart();
        ...
        dl.classList.add("section");
        dl.classList.add(info['class']);
        
        const title = DoxygenAwesomeAlerts.get_alert(info);
        dl.appendChild(title);

        const content = document.createElement("dd");
        for (let child of Array.from(blockquote.children)) {
          content.appendChild(child.cloneNode(true));
        }
        dl.appendChild(content);

        blockquote.replaceWith(dl);
      }
    })
  }
}